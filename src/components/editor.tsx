      ...
      <div className="relative bg-gray-200" style={{ width: '960px', height: '540px' }}>
        {editorContext.elements.map(el => (
          <Rnd
            ...
            onDragStop={(e, d) => editorContext.updateElement(el.id, d.x, d.y, el.width, el.height, el.text!, el.fontSize!, el.isBold!)}
            onResizeStop={(e, direction, ref, delta, position) => {
              editorContext.updateElement(el.id, position.x, position.y, ref.offsetWidth, ref.offsetHeight, el.text!, el.fontSize!, el.isBold!)
            }}
            bounds="parent"
          >
            <div className="relative border border-black bg-white flex items-center justify-center no-deselect" style={{ width: '100%', height: '100%' }} >
              {el.type === 'text' ? <span className="w-full h-full whitespace-pre-wrap no-deselect" onDoubleClick={() => editorContext.handleDoubleClick(el.id)} onBlur={(e) => editorContext.updateElement(el.id, el.x, el.y, el.width, el.height, e.target.textContent!, el.fontSize!, el.isBold!)} contentEditable={editorContext.isEditing && editorContext.focusedElement === el.id} style={{ fontSize: `${el.fontSize}px`, fontWeight: el.isBold ? 'bold' : 'normal' }}>{el.text}</span> : el.type === 'rect' ? 'Rectangle' : <img src={el.src} alt="Uploaded" className="pointer-events-none" style={{ width: '100%', height: '100%' }} />}
              {el.type === "text" ? editorContext.focusedElement === el.id && (
                <div className="absolute bottom-0 right-0 flex flex-row h-8 no-deselect">
                  <div className="flex flex-row no-deselect">
                    <button className=" bg-black text-white px-2 h-8 no-deselect" onClick={() => { editorContext.decreaseFontSize(el.fontSize!); editorContext.updateElement(el.id, el.x, el.y, el.width, el.height, el.text!, editorContext.fontSize, el.isBold!) }}><MinusSquare /></button>
                    <span className=" bg-black text-white px-2 h-8 flex items-center no-deselect">{el.fontSize}</span>
                    <button className="bg-black text-white px-2 h-8 no-deselect" onClick={() => { editorContext.increaseFontSize(el.fontSize!); editorContext.updateElement(el.id, el.x, el.y, el.width, el.height, el.text!, editorContext.fontSize, el.isBold!) }}><PlusSquare /></button>
                    <button className="bg-black text-white px-2 h-8 no-deselect" onClick={() => { editorContext.toggleBold(); editorContext.updateElement(el.id, el.x, el.y, el.width, el.height, el.text!, el.fontSize!, !editorContext.isBold) }}><Bold /></button>
                  </div>
                  <button
